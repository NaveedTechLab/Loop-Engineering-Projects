...
# Chrome discovery is cross-platform: env override, then macOS, then common Windows
# install paths, then PATH lookup for Linux binary names. This file was only ever
# tested with Chrome at the macOS path, so on every other OS every Chrome-dependent
# check (M12/M14/M16) silently no-op'd to "not measured" / fail — not a page defect.
CHROME = next((c for c in (
    os.environ.get("CHROME"),
    "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome",
    r"C:\Program Files\Google\Chrome\Application\chrome.exe",
    r"C:\Program Files (x86)\Google\Chrome\Application\chrome.exe",
    shutil.which("google-chrome"), shutil.which("chromium-browser"), shutil.which("chromium"),
...
if os.path.exists(CHROME):
    # --screenshot needs an ABSOLUTE path: this Chrome build silently no-ops with
    # "Access is denied" on a relative one (a platform quirk, not a permissions issue —
    # confirmed by writing successfully to the identical absolute path). Match the
    # os.path.abspath() already used for the file:// URL on the next line.
    r=subprocess.run([CHROME,"--headless","--disable-gpu","--enable-logging=stderr","--v=1",
        f"--screenshot={os.path.abspath(D)}/shot.png","--window-size=1200,900","--hide-scrollbars",
